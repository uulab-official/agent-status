...
- 🚧 Claude — `detect()` done, `fetch_status()` TODO (target ★★★☆☆ via `~/.claude` state, ★★☆☆☆ fallback via claude.ai). Investigated thoroughly: `~/.claude/sessions/*.json` is process metadata only, `session-env/` is empty, `stats-cache.json` has historical daily counts but no live 5-hour-window percentage — the real-time number genuinely isn't cached locally by the CLI.
- 🚧 OpenAI/ChatGPT — `detect()` done, `fetch_status()` TODO (target ★★★★★ via `/v1/usage` for API spend, ★★☆☆☆ for ChatGPT plan caps)
- 🚧 Gemini — `detect()` done, `fetch_status()` TODO (target ★★★★★ via AI Studio, ★★★☆☆ via CLI)
- ✅ Cursor — connectivity upgraded from a bare TODO to a real check (★★★☆☆ CLI log via `cursor-agent status`, same safe-CLI pattern as Codex/Copilot). Verified live: shows `🟢 Online`. Actual Premium/Slow quota numbers (★★★★☆ dashboard) are still 🚧 TODO — `cursor-agent`'s CLI surface has no `usage`/`limits` subcommand, only `status`/`whoami`.
