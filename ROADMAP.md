...
- 🚧 Google Antigravity (new provider, `crates/providers/antigravity`) — detection only (`~/.antigravity` config dir). No `antigravity` CLI exists on `$PATH` to shell out to for a login-status check (unlike Codex/Cursor/Copilot), and the only auth state on disk is `~/.antigravity_cockpit/credentials.json` — a credential file this project won't open directly (same line drawn for Claude's Keychain case and Cursor's dashboard cookie). `fetch_status()` reports `Unknown` rather than guessing `Online` from directory presence alone, so it's currently invisible in the popover (same as Copilot's blocked state).
